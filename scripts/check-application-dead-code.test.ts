...
} from "./check-application-dead-code";
...
describe("DI 容器实例化类收集", () => {
	it("应当收集 new 表达式中的类名", () => {
		...
		expect([...instantiated].sort()).toEqual(["GetCollectionsUseCase", "Logger"]);
	});
...
describe("应用层死代码检查", () => {
	it("当类未在 DI 容器实例化时，应当报死代码", () => {
		...
		const errors = checkDeadCode(classes, new Set(["GetCollectionsUseCase"]));
